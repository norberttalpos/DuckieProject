...
def hyperopti():
    ...
    history = model_best.fit(X_train, Y_train, epochs=50, validation_split=0.15, callbacks=[early_stopping], verbose=1)

    eval_result = model_best.evaluate(X_test, Y_test)
    print("[test loss, test accuracy]:", eval_result)

    #model_best.save("/tmp/model")
    keras.models.save_model(model_best,"/tmp/model12")
    
    keras.models.save_model(best_model,"/tmp/model13")
