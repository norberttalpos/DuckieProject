...
def build_model(hp):
    #model = Sequential()
    #keras.models.load_model(model,"/tmp/modelHiperoptiTemp")
    #return model
    model = Sequential()
...
def hyperopti():
    ...
    history = model_best.fit(X_train, Y_train, epochs=500, validation_data=(X_valid, Y_valid), callbacks=[early_stopping, reduce_lr, checkpoint, change_lr], verbose=1)

    eval_result = model_best.evaluate(X_test, Y_test)
    print("[test loss, test accuracy]:", eval_result)

    #model_best.save("/tmp/model")
    keras.models.save_model(model_best,"/tmp/modelBestTrain2")
    
    #keras.models.save_model(best_model,"/tmp/model")
    #print(model_best.predict(X_train[0]))
    
    #with open("History2.json","wb") as file_pi:
     #   pickle.dump(history.history,file_pi)

    #print(history
    np.save("his.npy",history.history)
    
    return model_best


result = hyperopti()
model = keras.models.load_model("/tmp/modelBestTrain2")
print(model.summary())
history = np.load("his.npy",allow_pickle="TRUE").item()
print(history)
