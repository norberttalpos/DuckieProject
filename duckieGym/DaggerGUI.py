...
class HumanDriver:
    ...
    def __init__(self, env, max_episodes, max_steps, log_file=None, downscale=False,playback=True,filter_bad_data=False):
        ...
        self.model = load_model("/tmp/dagger2")
        self.learner=DaggerLearner(self.model)
    ...
    def update(self,dt,env):
        ...
        if (self.first):
            self.first=False
            (self.obs, reward, done, info) = self.env.step([0.0,0.0])
        #img123 = np.reshape(preprocess_image(self.obs), (1, img_dim[0], img_dim[1], img_dim[2]))
        action = self.learner.predict(env,self.obs)
        #! GO! and get next
        ...
        if reward != -1000:
            ...
            if ((reward > self.last_reward-0.02) or not self.filter_bad_data):
                ...
                obs_distorted_DS = self.image_resize(self.obs, width=640)
